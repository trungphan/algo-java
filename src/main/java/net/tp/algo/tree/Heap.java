...
public class Heap<E> {
	...
	public Heap(Comparator<? super E> comparator, Collection<E> elements) {
		this.comparator = comparator;
		if (elements != null) {
			this.elements = new Entry[elements.size()];
			for (E item : elements) {
				this.elements[n] = new Entry<E>(this, item, n);
				n++;
			}
			heapify();
		}
	...
	private void siftUp(int i) {
		...
		while (true) {
			...
			if (compare(v.item, elements[pi].item) >= 0) {
				break;
			}
	...
	private void siftDown(int i) {
		...
		while (true) {
			...
			if (ci < n - 1 && compare(elements[ci].item, elements[ci+1].item) > 0) {
				ci++;
			}
			
			if (compare(v.item, elements[ci].item) <= 0) {
				break;
			}
	...
	/**
	 * 
	 * This operation takes O(log(n)) times. The client can keep the ref to remove the item later.
	 ...
	 * The client is encouraged to keep the ref when adding an item into this heap instead of finding the ref
	...
	public Ref find(E element) {
		for (int i = 0; i < n; i++) {
			if (compare(elements[i].item, element) == 0) {
				return elements[i];
			}
		}
		return null;
	...
	private int compare(E item1, E item2) {
		return comparator != null ? comparator.compare(item1, item2) : ((Comparable<E>)item1).compareTo(item2);
	}
	...
	private static class Entry<K> implements Ref {
		...
		private Entry(Heap<K> heap, K item, int pos) {
			if (item == null) {
				throw new NullPointerException();
			}
			this.heap = heap;
	...
	public static class TestCase {
		...
		public void heapify_one_element() {
			Heap<Integer> heap = new Heap<>(new NaturalComparator<Integer>(), Arrays.asList(5));
		...
		public void remove_ref_of_one_element_heap() {
			Heap<Integer> heap = new Heap<Integer>(new NaturalComparator<Integer>());
			Ref ref = heap.add(1);
